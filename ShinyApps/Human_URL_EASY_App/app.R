...
#              "plotly","readr","enrichR","ggrepel","tidyr","tools","shinycssloaders", "Hmisc")
...
ProjectName <- 'Human URL EASY App'


##--User Input File Names--##

#expression data
#expr_file <- 'GSE123813_bcc_logNormalized_data.txt'
expr_file <- ''

#url_expr_file <- 'https://github.com/shawlab-moffitt/ADIOS_SmallData/raw/main/GSE123813_bcc_expr.txt.zip'
url_expr_file = ''
#expr_filename = 'GSE123813_bcc_expr.txt'
expr_filename = ''
#meta data
#url_meta_file <- 'https://raw.githubusercontent.com/shawlab-moffitt/ADIOS_SmallData/main/GSE123813_bcc_meta_mod.txt'
url_meta_file = ''
meta_file <- 'GSE123813_bcc_meta_mod.txt'
...
## Expression Data -------------------------------------------------------------
#expr_orig <- as.data.frame(read_delim(expr_file, delim = '\t', col_names = T))


#temp <- tempfile()
#download.file(url_expr_file,temp)
#expr = as.data.frame(read_delim(unz(temp, expr_filename), delim = '\t', col_names = T))
#expr = as.data.frame(read_delim(unz(temp, expr_filename), delim = '\t', col_names = T))
#
# expr <- read_delim(getZip(url_expr_file), delim = '\t', col_names = T)
# unlink(temp)
#
...
#
# ## Meta Data -------------------------------------------------------------
# meta <- read.delim(url(url_meta_file), sep = '\t', header = header, strip.white = T)
...
About_tab <- tabPanel("Intro/Methodology",
                      fluidPage(
                        mainPanel(
                          textInput("text1", "ExpressionMatrix", ""),
                          textInput("text2", "MetaFile", ""),
                          h3("Introduction"),
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    if (!is.null(query[['expr']]) && !is.null(query[['meta']])) {
      updateTextInput(session, "text1", value = query[['expr']])
      updateTextInput(session, "text2", value = query[['meta']])
      url_expr_file = query[['expr']];
      url_meta_file = query[['meta']];
      ...
      SubCol_reactVal(Subsetting_Feature)
      SubCrit_reactVal(1)
      metacol_reactVal(Feature_Selected)
    }
